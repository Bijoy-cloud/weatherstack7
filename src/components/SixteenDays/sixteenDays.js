...
function SixteenDays(props) {
  ...
  useEffect(() => {
    if (input != null) {
      axios
        .get(
          `https://api.weatherbit.io/v2.0/forecast/daily?city=${input}&key=${URL}`
        )
        .then((response) => {
          let temp = [];
          let icon = [];
          ...
          <div className="container-wrapper">
            <div className="container flex" key={index}>
              <div className="container-one ">
                <h3>
                  {new Date(data.datetime[index]).toLocaleString("en-us", {
                    weekday: "long",
                  })}
                  ,{new Date(data.datetime[index]).getDate()}
                  {new Date(data.datetime[index]).toLocaleString("default", {
                    month: "short",
                  })}
                </h3>
                <p>
                  {data.description[index]}, {data.temp[index]}
                </p>
              </div>
